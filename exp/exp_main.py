...
class Exp_Main(Exp_Basic):
    ...
    def _build_model(self):
        ...
        model = model.to(self.device)
        # summary(model,input_size=(16, 96,137))
        return model
    ...
    def test(self, setting, test=0):
        ...
        wandb.log({"mse": np.mean(mse)})
        wandb.finish()
        print('wandb ok ')
        print('mse:{}, mae:{}, rmse:{}, mape:{}, mspe:{}, corr:{}, rse:{}'.format(np.mean(mse), np.mean(mae), np.mean(rmse), np.mean(mape), np.mean(mspe), np.mean(corr), np.mean(rse)))
