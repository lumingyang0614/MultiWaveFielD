...
class ConvLayer2D(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.sampleConv is not None:
            y = self.sampleConv(y)
            # print("Shape of y after sampleConv:", y.shape)
            
        if self.pool is not None:
            y = self.pool(y)
            # print("Shape of y after pooling:", y.shape)
            
        x = self.dropout(self.downConv(x))
        # print("Shape of x after downConv and dropout:", x.shape)
        
        x = self.activation1(x)
        # print("Shape of x after activation1:", x.shape)
        
        x = self.dropout(self.actConv(x))
        # print("Shape of x after actConv and dropout:", x.shape)
        
        x = self.activation2(x)
        # print("Shape of x after activation2:", x.shape)
        
        x = x + y
        # print("Shape of x after addition with y:", x.shape)
